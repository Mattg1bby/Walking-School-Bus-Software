...
void handleInterrupt(void *arg) {
    ...
    if (gpio_status & BIT(4)) {
        ...
        if(inter_count == 0){
            ...
            wifi_fpm_do_wakeup();                                       //Wake Device Up
        ...
        else //if (inter_count == 1)
        {
            ...
            wifi_fpm_open();								        	//Enable Force Sleep Mode
			wifi_fpm_do_sleep(FPM_SLEEP_MAX_TIME);	                    //Put device into Sleep
            //Debounce Time 250ms
            os_delay_us(50000);os_delay_us(50000);os_delay_us(50000);os_delay_us(50000);os_delay_us(50000);
        ...
        //Clear interrupt status and Reenable
